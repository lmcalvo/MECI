...
LassoFISTA <- function(y,X,lambda){
  ...
  tolerancia=0.00001 # Tolerancia - Criteria de parada: Diferencias entre el valor de la función objetivo en 2 iteraciones consecutivas
...
c <- 1.1
g <- 0.1/log(max(p,n))
lambda <- c*qnorm(1-0.5*g/p)/sqrt(n)
Lassofit <- LassoFISTA(y,X,lambda)
```
#### Error L1
